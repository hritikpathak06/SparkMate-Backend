...
// Swipe Right
export const swipeRight = async (req: any, res: any) => {
  try {
    const { likedUserId } = req.params;
    const current_user: any = await User.findById(req.userId);
    ...
    if (likedUser.likes.includes(current_user._id)) {
      ...
      if (currentUserSocketId) {
        console.log(
          `Emitting new match to currentUser: ${currentUserSocketId}`
        );
        ...
        console.log(
          `currentUserSocketId not found for user ${current_user._id}`
        );
      }
...
// Swipe Left
export const swipeLeft = async (req: any, res: any) => {
  try {
  ...
  try {
    ...
    const users = await User.find({
      $and: [
        ...
        {
          gender:
            current_user?.genderPreference === "both"
              ? { $in: ["male", "female"] }
